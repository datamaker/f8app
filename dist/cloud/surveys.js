...
Parse.Cloud.define('send_surveys', function (request, response) {
  ...
  var attendees = new Parse.Query(Attendance).equalTo('agenda', agenda).notEqualTo('sent', true).find();
  var survey = new Parse.Query(Survey).equalTo('session', agenda).first();

  Parse.Promise.when(attendees, survey, new Parse.Query(Agenda).get(sessionId)).then(sendSurveys).then(function (value) {
...
Parse.Cloud.define('surveys', function (request, response) {
  ...
  new Parse.Query(SurveyResult).equalTo('user', user).equalTo('rawAnswers', null).include('survey').include('survey.session').find().then(toSurveys).then(function (value) {
    response.success(value);
  }, function (error) {
    response.error(error);
  });
...
Parse.Cloud.define('submit_survey', function (request, response) {
  ...
  new Parse.Query(SurveyResult).equalTo('user', user).equalTo('objectId', params.id).find().then(function (results) {
    if (results.length === 0) {
